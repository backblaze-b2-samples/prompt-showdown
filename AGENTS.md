...
**Prompt Showdown** compares N prompt variants across a shared input set: every
output is generated through an LLM (NVIDIA NIM, via the Genblaze SDK), scored by
an LLM judge (and optionally a human), and the whole run is preserved on
Backblaze B2 as the durable, queryable system of record.
...
### Showdown-specific modules

```
services/api/app/
  types/showdown.py      PromptVariant, ShowdownInput, RunCell, ShowdownRun, RunSummary, ...
  config/settings.py     B2_* (standardized) + nvidia_api_key + showdown_*_model
  repo/genblaze_repo.py  Genblaze Pipeline / NvidiaChatProvider / chat / S3 sink — ALL genblaze imports live here
  repo/showdown_store.py B2 run-record store (boto3): put/get/list under the scoped "showdowns/" prefix
  repo/b2_client.py      shared boto3 S3 client + file-explorer / upload helpers
  service/showdown.py    orchestrate a run (grid -> generate -> judge -> assemble -> persist), list/get/export/score
  service/scoring.py     leaderboard + aggregate-stats math (pure functions)
  runtime/showdown.py    POST /runs, GET /runs, GET /runs/{id}, GET /runs/{id}/export, POST /runs/{id}/scores

apps/web/src/
  app/showdowns/         /showdowns (history), /showdowns/new (form), /showdowns/[id] (grid + leaderboard)
  components/showdowns/   run-history, new-showdown-form, run-detail, showdown-grid, leaderboard
  components/dashboard/   showdown-stats-cards, wins-chart, recent-runs-table
  lib/{api-client,queries,showdown}.ts  data layer + form helpers
```

**Provider orchestration is Genblaze, not bare SDKs.** All AI-provider calls
route through `genblaze-core` + `genblaze-nvidia[chat]` + `genblaze-s3`, and
every genblaze import stays inside `services/api/app/repo/`. Storage stays boto3.
...
- No `genblaze*` imports outside `repo/` (guarded by `tests/test_genblaze_signatures.py`)
...
| No genblaze outside repo/ | `tests/test_genblaze_signatures.py::test_genblaze_imports_contained_in_repo` |
| Genblaze call shapes (SDK-drift guard) | `tests/test_genblaze_signatures.py` (no-network) |
