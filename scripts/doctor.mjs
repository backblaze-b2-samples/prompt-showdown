...
function checkEnv() {
  ...
  if (placeholders.length > 0) {
    fail(
      ...
      "Edit .env and replace placeholders with your real B2 credentials (https://secure.backblaze.com/app_keys.htm?utm_source=github&utm_medium=referral&utm_campaign=ai_artifacts&utm_content=b2ai-prompt-showdown)",
    );
  }

  // NVIDIA NIM key powers generation + judging. Warn (not fail) so the file
  // explorer / upload still run without it; showdown runs need it.
  if (!env.NVIDIA_API_KEY || env.NVIDIA_API_KEY === "your_nvidia_api_key") {
    warn(
      "NVIDIA_API_KEY is missing or still a placeholder",
      "Get a free key at https://build.nvidia.com and set NVIDIA_API_KEY in .env. Showdown runs (generation + judging) require it; the file explorer works without it.",
    );
  }
}
