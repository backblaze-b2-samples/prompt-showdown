...
export async function createRun(req: CreateRunRequest) {
  return apiFetch<ShowdownRun>("/runs", {
    method: "POST",
...
export async function getShowdownStats() {
  return apiFetch<ShowdownStats>("/runs/stats");
}

export async function getVariantWins() {
  return apiFetch<VariantWins[]>("/runs/stats/wins");
}

export async function getRun(runId: string) {
  return apiFetch<ShowdownRun>(`/runs/${encodeURIComponent(runId)}`);
}

export function exportRunUrl(runId: string) {
  return `${API_BASE}/runs/${encodeURIComponent(runId)}/export`;
}

export async function scoreCell(runId: string, req: ScoreRequest) {
  return apiFetch<ShowdownRun>(`/runs/${encodeURIComponent(runId)}/scores`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(req),
  });
