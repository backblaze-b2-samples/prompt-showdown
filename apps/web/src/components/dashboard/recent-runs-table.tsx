      ...
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <EmptyState
            icon={Swords}
            title="No runs yet"
            description="Create a showdown to compare your prompt variants."
          />
        ) : (
              ...
              {recent.map((run) => (
                <TableRow key={run.run_id} className="table-row-hover">
                  <TableCell className="font-medium">
                    <Link href={`/showdowns/${run.run_id}`} className="hover:underline">
                      <span className="truncate block">{run.title}</span>
                    </Link>
                  </TableCell>
